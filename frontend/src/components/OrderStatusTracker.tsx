  ...
  if (isCancelled) {
    return (
      <div className="flex items-center justify-center gap-3 py-4">
        ...
        <div>
          <p className="font-display text-sm tracking-widest uppercase text-destructive">Order Cancelled</p>
          <p className="text-xs font-body text-muted-foreground">This order has been cancelled.</p>
        </div>
      </div>
    );
  }
            ...
            <div key={step.status} className="flex flex-col items-center gap-2 z-10 flex-1">
              ...
              <span
                className={`text-[10px] font-display tracking-widest uppercase text-center leading-tight ${
                  isActive ? 'text-gold' : isCompleted ? 'text-gold/70' : 'text-muted-foreground'
                }`}
              >
                {step.label}
              </span>
